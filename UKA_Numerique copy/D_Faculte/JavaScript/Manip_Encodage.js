...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    if (container !== null) {
        ...
        Liste_Etudiants();
        // NE PAS appeler Afficher_EC_aligne_encodage() au démarrage (aucun semestre sélectionné)
...
async function Afficher_EC_aligne_encodage() {
  ...
  // Mettre à jour les statistiques (compter les ECs réels et les côtes avec valeurs)
  // Utiliser setTimeout pour s'assurer que le DOM est bien rendu
  setTimeout(() => {
    console.log('🔄 Mise à jour des stats Encodage...');
    updateStats();
  }, 200); // Augmenter à 200ms pour être sûr
}
...
function updateStats() {
  ...
  // Compter les étudiants (lignes du tbody)
  const nbEtudiants = document.querySelectorAll('#table_encodage tbody tr').length;
  
  // Compter les ECs réels (colonnes td dans tr1, en excluant N°, Nom, et label "EC")
  const thead = document.querySelector('#table_encodage thead');
  const tr1 = thead ? thead.querySelectorAll('tr')[0] : null; // 1ère ligne (index 0)
  let nbECs = 0;
  if (tr1) {
    const allCells = tr1.querySelectorAll('td');
    // tr1 contient: N° (rowSpan=3), Nom (rowSpan=3), "EC" (label sticky) + N noms d'ECs
    nbECs = allCells.length - 3; // -3 pour exclure N°, Nom, et label "EC"
    console.log(`🔍 Encodage - tr1 a ${allCells.length} cellules, ECs = ${nbECs}`);
  }
  
  // Compter les côtes remplies (cellules éditables avec valeur non vide)
  const allEditableCells = document.querySelectorAll('#table_encodage tbody .cell-editable');
  const nbCotesRemplies = Array.from(allEditableCells)
    .filter(cell => {
  ...
  console.log(`📊 Stats Encodage - Étudiants: ${nbEtudiants}, ECs: ${nbECs}, Côtes: ${nbCotesRemplies}/${allEditableCells.length}`);
  ...
  if (countCotesElement) countCotesElement.textContent = nbCotesRemplies;
}
