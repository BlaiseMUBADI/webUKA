...
var RatSem1 = document.getElementById("RatSem1");
...
prom.addEventListener('change',(event) => {
  ...
  {
    var lien='D_Finance/Liste_Paiement/API/APISelect_FA_Fixe.php?code_promo='+code_prom+'&Id_annee_acad='+idAnnee_Ac;  
  ...
  function AfficherListePaiement(code_prom,idAnnee_Ac)
{
    ...
    TabListe.classList.add("table", "table-bordered");
  ...
  tr1.style = "background-color:midnightblue; color:white; text-align:center;";
    ...
    var url='D_Finance/Liste_Paiement/API/API_Select_Liste_Paie.php?code_promo='+code_prom+'&Id_annee_acad='+idAnnee_Ac;  
      ...
      data.forEach(infos =>
        {
          // Création de TR
              ...
              var tdResteFA = document.createElement("td");
              var tdEnrolMiss = document.createElement("td");
              var tdEnrolRatSem1 = document.createElement("td");
              ...
              tdResteFA.textContent=document.getElementById("FA").innerText-infos.Montant;
              tdEnrolMiss.textContent=infos.Enrol_S_1;
              tdEnrolRatSem1.textContent=infos.Rat_Sem_1;
           ...
           //.............................................
              ...
              else if(RatSem1.checked){session="Rattrapage-Sem-1";}
              ...
              if(session="Mi-Session" && MiSession.checked)
               ...
               else if(session="Rattrapage-Sem-1" && RatSem1.checked)
                {
                  enrolement=document.getElementById("Enrolement").innerText;
                  montanpaye= parseFloat(document.getElementById("tranche").innerText);
                  if (infos.Montant >= montanpaye && infos.Rat_Sem_1 == enrolement)
                    {
                      tdObs.textContent="En ordre";
                      countEnOrdre++; // Incrémenter le compteur "En ordre"
                    } 
             ...
             tr.appendChild(tdnum);
              ...
              tr.appendChild(tdResteFA);
              tr.appendChild(tdEnrolMiss);
              tr.appendChild(tdEnrolRatSem1);
...
function rechercherAgent(texteRecherche) {
  const table = document.querySelector("#TabListePaie");
  if (table) {
    ...
    if (tbody) {
      ...
      lignes.forEach(ligne => {
        const tdNom = ligne.querySelector("td:nth-child(3)"); // Colonne du nom
        const tdPrenom = ligne.querySelector("td:nth-child(4)"); // Colonne du prénom (ou autre champ)

        if (tdNom && tdPrenom) {
          const texteNom = tdNom.textContent.toLowerCase();
          const textePrenom = tdPrenom.textContent.toLowerCase();
          const texteComplet = texteNom + " " + textePrenom;

          if (texteComplet.includes(texteRecherche.toLowerCase())) {
            ligne.style.display = "";
          } else {
      ...
      console.error("Aucun tbody trouvé dans #TabListePaie.");
    }
  } else {
    console.error("Aucun élément trouvé avec l'ID #TabListePaie.");
  }
}
