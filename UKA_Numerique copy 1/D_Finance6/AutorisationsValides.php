...
<div class="container">
    <div class="d-flex align-items-center justify-content-between mb-2 p-2 gap-3 bg-dark text-white rounded">
        ...
        <button class="btn btn-danger" onclick="showTab('dejaServie')" hidden>🚫 Déjà servie</button>
        <h2 class="m-0 text-center flex-grow-1">Autorisations de dépenses</h2>
        
        <button class="btn btn-danger" onclick="showTab('rejete')">Demandes Rejetées</button>
    </div>
                    ...
                    <tbody>
                        ...
                        <tr class="table-secondary">
                            ...
                            <td colspan="2">
                                ...
                                <button id="btnDecaisser" 
                                    
                                        class="btn btn-success btn-decaisser" 
                                    data-devise="<?php echo $devise; ?>"
                                    data-dejaservie="<?php echo $deja_servie ? '1' : '0'; ?>"
...
window.addEventListener("DOMContentLoaded", function () {
    ...
    // Fonction d’affichage du message de clôture sous chaque tableau concerné
    function afficherMessageCloture(devise, cloturee) {
        const cartes = document.querySelectorAll(`.slider-item[data-devise="${devise}"]`);
        cartes.forEach(carte => {
            carte.querySelectorAll(".alerte-cloture").forEach(el => el.remove());
            if (cloturee) {
                const alertDiv = document.createElement("div");
                alertDiv.className = "alert alert-warning alerte-cloture mt-2 text-center";
                alertDiv.textContent = `⚠️ Les opérations de décaissement en ${devise} sont actuellement clôturées.`;
                carte.appendChild(alertDiv);
            }
        });
    }

    // Fonction d’activation/désactivation des boutons selon la devise et le statut
    function majBoutons(devise, cloturee) {
        const boutons = document.querySelectorAll(`.btn-decaisser[data-devise="${devise}"]`);
        boutons.forEach(btn => {
            const dejaServie = btn.dataset.dejaservie === "1"; // Vérifie si déjà servie
            const doitDesactiver = cloturee || dejaServie;     // Si l’un des deux est vrai
            btn.disabled = doitDesactiver;
            btn.style.opacity = doitDesactiver ? "0.6" : "1";
            btn.style.cursor = doitDesactiver ? "not-allowed" : "pointer";
            if (cloturee && !dejaServie) {
                btn.textContent = "🔒 Clôturé";
            } else if (dejaServie) {
                btn.textContent = "🚫 Déjà servie";
            } else {
                btn.textContent = "💸 Décaisser";
            }
        });
    }

    // Vérifie la clôture d’une devise via ton API
    function verifierCloture(devise) {
        return fetch("D_Finance/API/verifier_cloture.php", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ devise: devise, date: aujourdHui })
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                const cloturee = data.clotureExiste;
                majBoutons(devise, cloturee);
                afficherMessageCloture(devise, cloturee);
            }
        })
        .catch(err => console.error(`Erreur vérif clôture ${devise}:`, err));
    }
