...
$date_jr = date('Y-m-d'); // Utiliser la date reçue ou celle du jour

$query_usd = "SELECT SUM(decaissement_caisse.Montant) as total_dec_usd 
...
$Soldedec = $soldeDec['total_dec_usd'] ?? 0;
...
$query_usd = "SELECT SUM(decaissement_caisse.Montant) as total_dec_usd_jr 
              ...
              INNER JOIN numero_piece 
                  ON decaissement_caisse.Num_piece = numero_piece.numero_pce
              WHERE decaissement_caisse.Statut = :statut 
              AND date(decaissement_caisse.Date_Oper) =:date_oper
              AND decaissement_caisse.Num_piece LIKE :prefix";
...
$stmt_usd->execute();
$soldeDec_jr = $stmt_usd->fetch(PDO::FETCH_ASSOC);
$Soldedec_usd_jr = $soldeDec_jr['total_dec_usd_jr'] ?? 0;

$prefix = "Dec_cdf_";
$query_cdf = "SELECT SUM(decaissement_caisse.Montant) as total_dec_cdf 
              FROM decaissement_caisse
              INNER JOIN numero_piece 
                  ON decaissement_caisse.Num_piece = numero_piece.numero_pce
              WHERE decaissement_caisse.Statut = :statut 
...
$Soldedec_cdf = $soldeDec_cdf['total_dec_cdf'] ?? 0;

$query_usd = "SELECT SUM(decaissement_caisse.Montant) as total_dec_cdf_jr 
              FROM decaissement_caisse
              INNER JOIN numero_piece 
                  ON decaissement_caisse.Num_piece = numero_piece.numero_pce
              WHERE decaissement_caisse.Statut = :statut 
...
$stmt_cdf = $con->prepare($query_usd);
$prefixLikeCDF = $prefix . '%';
...
$stmt_cdf->execute();
$soldeDec_jr = $stmt_cdf->fetch(PDO::FETCH_ASSOC);
$Soldedec_cdf_jr = $soldeDec_jr['total_dec_cdf_jr'] ?? 0;
...
echo json_encode([
    ...
    "Solde__dec_usd_jr" => number_format($Soldedec_usd_jr, 2),
    "Solde__dec_cdf_jr" => number_format($Soldedec_cdf_jr, 2)
