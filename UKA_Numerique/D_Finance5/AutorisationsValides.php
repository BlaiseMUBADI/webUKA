...
window.addEventListener("DOMContentLoaded", function () {
    ...
    function afficherMessageCloture(devise, cloturee) {
        // Sélectionner tous les tableaux contenant des autorisations pour cette devise
        const cartes = document.querySelectorAll(`.slider-item[data-devise="${devise}"]`);
        cartes.forEach(carte => {
            // Supprimer les anciens messages s’il y en a
            carte.querySelectorAll(".alerte-cloture").forEach(el => el.remove());

            // Si clôturé → ajouter une alerte sous le tableau
            if (cloturee) {
    ...
    // Fonction d’activation/désactivation des boutons selon la devise
    function majBoutons(devise, cloturee) {
        const boutons = document.querySelectorAll(`.btn-decaisser[data-devise="${devise}"]`);
        boutons.forEach(btn => {
            btn.disabled = cloturee;
            btn.style.opacity = cloturee ? "0.6" : "1";
            btn.style.cursor = cloturee ? "not-allowed" : "pointer";
        });
    }
